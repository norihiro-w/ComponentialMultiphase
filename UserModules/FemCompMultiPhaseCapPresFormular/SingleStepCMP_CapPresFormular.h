...
*
*
* \file SingleStepCMP_CapPresFormular.h
*
* Created on 2015-03-12 by Yonghui HUANG
*/

#ifndef SINGLE_STEP_CMP_CAPPRESFORMULAR_H 
#define SINGLE_STEP_CMP_CAPPRESFORMULAR_H 
...
>
class SingleStepCMP_CapPresFormular
...
public:
	...
	*/
	SingleStepCMP_CapPresFormular(MyDiscreteSystem*      dis,
	...
	*/
	virtual ~SingleStepCMP_CapPresFormular()
	{
		/*
...
private:
	DISALLOW_COPY_AND_ASSIGN(SingleStepCMP_CapPresFormular);
...
template <
	...
	class T_USER_NON_LINEAR_SOLUTION
>
SingleStepCMP_CapPresFormular<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::SingleStepCMP_CapPresFormular(MyDiscreteSystem*                dis,
                           UserFemProblem*                  problem,
						   UserNonLinearSolution*           nlin_solution,
						   UserFunctionData*                function_data)
...
_problem(problem), _nlin_solution(nlin_solution), _discrete_system(dis), _function_data(function_data)
{
	...
	INFO("->Setting up a solution algorithm for CMP_CapPresFormular problem.");
	...
	for (i_var = 0; i_var < n_var; i_var++) {
		...
		for (i = 0; i < var->getNumberOfDirichletBC(); i++) {
			...
			for (j = 0; j < list_bc_nodes.size(); j++)
			{
				...
				// set BC values
				if (i_var == 0)
					_function_data->set_PN_node_values(node_id, node_value);
				else if (i_var == 1)
					_function_data->set_PC_node_values(node_id, node_value);
...
template <class T_USER_FUNCTION_DATA,
		  ...
		  class T_USER_NON_LINEAR_PROBLEM,
		  class T_USER_NON_LINEAR_SOLUTION>
int SingleStepCMP_CapPresFormular<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::solveTimeStep(const NumLib::TimeStep &t_n1)
{
	// print solving information
	INFO("--Solving nonlinear equation for the CMP_CapPresFormular process. \n");
	
	_nlin_solution->solveTimeStep(t_n1);

    // update the P and X vector
	_function_data->set_PN(_nlin_solution->getCurrentSolution(0));
    _function_data->set_PC(_nlin_solution->getCurrentSolution(1));
