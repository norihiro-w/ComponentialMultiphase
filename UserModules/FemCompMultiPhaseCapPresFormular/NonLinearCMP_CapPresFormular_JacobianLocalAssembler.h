...
#ifndef NON_LINEAR_CMP_CAPPRESFORMULAR_JACOBIAN_LOCAL_ASSEMBLER_H
#define NON_LINEAR_CMP_CAPPRESFORMULAR_JACOBIAN_LOCAL_ASSEMBLER_H
...
class NonLinearCMP_CapPresFormular_JacobianLocalAssembler : public NumLib::IElementWiseTransientJacobianLocalAssembler
{
public:
	typedef MathLib::LocalVector LocalVectorType;
	typedef MathLib::LocalMatrix LocalMatrixType;
	NonLinearCMP_CapPresFormular_JacobianLocalAssembler(FemLib::LagrangeFeObjectContainer* feObjects, T_FUNCTION_DATA* func_data, const MeshLib::CoordinateSystem &problem_coordinates)
		: _feObjects(*feObjects), _function_data(func_data), _problem_coordinates(problem_coordinates)
    {
		
	};

	virtual ~NonLinearCMP_CapPresFormular_JacobianLocalAssembler()
    {
		_function_data = NULL; 
	...
	/*
	* Construct inexact Jaocbian Matrix
	* ignore some derivitive coefficients 
	*/
	/*void assembly(const NumLib::TimeStep &time, const MeshLib::IElement &e, const DiscreteLib::DofEquationIdTable &, const MathLib::LocalVector & u1, const MathLib::LocalVector & u0, MathLib::LocalMatrix & localJ)
    {
		...
		LocalMatrixType _M = LocalMatrixType::Zero(n_dof, n_dof);
		LocalMatrixType _K = LocalMatrixType::Zero(n_dof, n_dof);
	...
	*/
	/*
	* Construct exact Jaocbian Matrix 
	*/
	void assembly(const NumLib::TimeStep &time, const MeshLib::IElement &e, const DiscreteLib::DofEquationIdTable &, const MathLib::LocalVector & u1, const MathLib::LocalVector & u0, MathLib::LocalMatrix & localJ)
	{
		size_t el(0);
...
private:
	...
	MathLib::LocalMatrix PN_gp; // gas phase pressure
	MathLib::LocalMatrix PC_gp;// capillary pressure
	MathLib::LocalMatrix PN_OLD_gp; // the value of primary variable P on each gauss point
	MathLib::LocalMatrix PC_OLD_gp;// the value of primary variable X on each gauss point
