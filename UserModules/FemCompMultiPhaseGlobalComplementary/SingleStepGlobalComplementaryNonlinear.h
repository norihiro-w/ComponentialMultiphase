...
template <
    ...
    >
class SingleStepGlobalComplementaryNonlinear
...
public:
    ...
    SingleStepGlobalComplementaryNonlinear(MyDiscreteSystem* dis, UserFemProblem* problem, T_USER_FUNCTION_DATA* user_data, MyNonlinearSolverFactory* nl_solver_factory=NULL);

    ///
    virtual ~SingleStepGlobalComplementaryNonlinear()
    {
...
private:
    DISALLOW_COPY_AND_ASSIGN(SingleStepGlobalComplementaryNonlinear);
...
template <
    ...
    >
SingleStepGlobalComplementaryNonlinear<T_USER_FEM_PROBLEM,T_USER_FUNCTION_DATA,T_LINEAR_SOLVER,T_NL_SOLVER_FACTORY>::SingleStepGlobalComplementaryNonlinear(MyDiscreteSystem* dis, UserFemProblem* problem, T_USER_FUNCTION_DATA* userData, MyNonlinearSolverFactory* nl_solver_factory)
      ...
      _problem(problem), _user_data(userData), _discrete_system(dis)
{
	...
	_f_r = new UserResidualFunction(dis, list_var, &_dofManager, problem->getEquation()->getResidualAssembler(), _user_data);
	_f_dx = new UserDxFunction(dis->getMesh(), list_var, _linear_eqs, &_dofManager, _user_data, problem->getEquation()->getJacobianAssembler());
...
template <
    ...
    class T_NL_SOLVER_FACTORY
    >
int SingleStepGlobalComplementaryNonlinear<T_USER_FEM_PROBLEM,T_USER_FUNCTION_DATA,T_LINEAR_SOLVER,T_NL_SOLVER_FACTORY>::solveTimeStep(const NumLib::TimeStep &t_n1)
{
    // time step
