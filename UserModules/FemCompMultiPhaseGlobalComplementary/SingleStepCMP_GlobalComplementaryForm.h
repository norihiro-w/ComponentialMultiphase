...
#ifndef SINGLE_STEP_CMP_GlobalComplementaryForm_H 
#define SINGLE_STEP_CMP_GlobalComplementaryForm_H 
...
>
class SingleStepCMP_GlobalComplementaryForm
...
public:
	...
	*/
	SingleStepCMP_GlobalComplementaryForm(MyDiscreteSystem*      dis,
	...
	*/
	virtual ~SingleStepCMP_GlobalComplementaryForm()
	{
		/*
...
private:
	DISALLOW_COPY_AND_ASSIGN(SingleStepCMP_GlobalComplementaryForm);
...
template <
	...
	class T_USER_NON_LINEAR_SOLUTION
>
SingleStepCMP_GlobalComplementaryForm<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::SingleStepCMP_GlobalComplementaryForm(MyDiscreteSystem*                dis,
                           UserFemProblem*                  problem,
						   UserNonLinearSolution*           nlin_solution,
						   UserFunctionData*                function_data)
...
_problem(problem), _nlin_solution(nlin_solution), _discrete_system(dis), _function_data(function_data)
{
	...
	INFO("->Setting up a solution algorithm for CMP_GlobalComplementaryForm problem.");
		...
		for (i = 0; i < var->getNumberOfDirichletBC(); i++) {
			...
			for (j = 0; j < list_bc_nodes.size(); j++)
			{
				...
				if (i_var == 0)
					...
					_function_data->set_S_node_values(node_id, node_value);
			}  // end of for j
		}  // end of for i
	}  // end of for i_var
...
template <class T_USER_FUNCTION_DATA,
		  ...
		  class T_USER_NON_LINEAR_PROBLEM,
		  class T_USER_NON_LINEAR_SOLUTION>
int SingleStepCMP_GlobalComplementaryForm<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::solveTimeStep(const NumLib::TimeStep &t_n1)
{
	// print solving information
	INFO("--Solving nonlinear equation for the CMP_GlobalComplementaryForm process. \n");
	...
	_function_data->set_S(_nlin_solution->getCurrentSolution(2));
