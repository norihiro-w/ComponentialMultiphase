			...
			std::vector<CKinBlob*>      &KinBlob_vector)
{
	...
	std::ifstream krc_file(krc_file_name.data(), std::ios::in);
	if (!krc_file.good())
		return false;
	...
	if ( !KinReact_vector.empty() )
	{
		...
		std::ifstream dbf_file(database_file_name.data(), std::ios::in);
		...
		for (size_t i = 0; i < length; i++)
		{
			...
			if (m_kr->getType() == "NULL")
			{
				...
				while (!dbf_file.eof() && found == 0)
				{
					...
					//  	break;
					dbf_file.getline(line, MAX_ZEILE);
					line_string = line;
					// keyword found
					if (line_string.find("#REACTION") != std::string::npos)
					{
...
//				if (m_pcs == NULL)
//				{
//					std::cout << " Warning: Component " << m_kr->bacteria_name
//					     <<
...
//				if (m_pcs == NULL)
//				{
//					std::cout << " Warning: Component " << m_kr->monod[i]->species
//					     <<
...
//				if (m_pcs == NULL)
//				{
//					std::cout << " Warning: Component " << m_kr->inhibit[i]->species
//					     <<
...
//				if (m_pcs == NULL)
//				{
//					std::cout << " Warning: Component " << m_kr->Isotope_light
//					     <<
...
//		if (ok == false)
//		{
//			std::cout << " Components missing, Stopping" << std::endl;
//			std::cout.flush();
//			exit(1);
//		}
//
...
//					for (size_t i = 0; i < length; i++)
//						if (m_name.compare(cp_vector[i]->compname) == 0)
//							m_kr->ProductionStoch[i] =
//							        m_kr->ProdStochhelp[k]->
//							        concentration;
//				}
//			} //if type == monod
...
//						if (m_name.compare(cp_vector[k]->compname) == 0)
...
//						for (size_t k = 0; k < length; k++)
//							if (m_name.compare(cp_vector[k]->compname) ==
//							    0)
//							{
...
//		for (size_t j = 0; j < length; j++)
//		{
//			std::string const& sp_name (cp_vector[j]->compname);
//			// HS, PCSGet not needed any more.
//			// m_pcs = PCSGet("MASS_TRANSPORT", sp_name);
//			// new style:
//			m_pcs = cp_vector[cp_name_2_idx[sp_name]]->getProcess();
//			if (m_pcs != NULL)
...
bool CKinReact::Read(std::ifstream* rfd_file)
{
	...
	while (!new_keyword)
	{
		...
		{
			line_str1 = readNonBlankLineFromInputStream(*rfd_file);
        ...
        // subkeyword found
		if (line_string.find("$EQ_CONSTANT") != string::npos)
		{
			in.str(readNonBlankLineFromInputStream(*rfd_file));
			in >> eq_const_k;
		...
		if (line_string.find("$MONODTERMS") != string::npos)
			while (!new_subkeyword)
			{
				...
				{
					ERR(" ERROR reading Monod Terms  - skipping");
					number_monod--;
					delete m_monod;
				}
				in.clear();
		...
		//....................................................................
		// subkeyword found
		if (line_string.find("$THRESHHOLDTERMS") != std::string::npos)
			while (!new_subkeyword)
			{
				index1 = (size_t)rfd_file->tellg();
				line_str1 = readNonBlankLineFromInputStream(*rfd_file);
				// Check for end of data block
				if ((line_str1.find("#") != std::string::npos) || (line_str1.find("$") != std::string::npos))
				{
					if (line_str1.find("#") != std::string::npos)
						new_keyword = true;
					rfd_file->seekg(index1); //Dateipointer zur�cksetzen, sonst ist das n�chste subkeyword weg
					break;
				}
				...
				if ((thresh_ord != -99.0) && (thresh_conc != -1.0e9))
				{
					...
					if (found == false)
					{
						std::cout << " WARNING: no matching MONOD SPECIES found in reaction ";
						std::cout << name << " for THRESHHOLD TERM SPECIES "
						     << thresh_species << std::endl;
					}
				}
				else
					ERR(" ERROR reading Threshhold Terms  - skipping");
				in.clear();
			}
		//....................................................................
		// subkeyword found
		if (line_string.find("$INHIBITIONTERMS") != std::string::npos)
			while (!new_subkeyword)
			{
				index1 = (size_t)rfd_file->tellg();
				line_str1 = readNonBlankLineFromInputStream(*rfd_file);
				// Check for end of data block
				if ((line_str1.find("#") != std::string::npos) || (line_str1.find(
				                                                      "$") !=
				                                              std::string::npos))
				{
					if (line_str1.find("#") != std::string::npos)
				...
				{
					delete m_inhibit;
					ERR(" ERROR reading Inhibition Terms  - skipping");
					number_inhibit--;
				}
		...
		//....................................................................
		// subkeyword found
		if (line_string.find("$PRODUCTIONTERMS") != std::string::npos)
			while (!new_subkeyword)
			{
				index1 = (size_t)rfd_file->tellg();
				line_str1 = readNonBlankLineFromInputStream(*rfd_file);
				...
				{
					ERR(" ERROR reading Production Terms  - skipping");
					number_production--;
				}
				in.clear();
			}
		...
		//....................................................................
		// subkeyword found
		if (line_string.find("$PRODUCTIONSTOCH") != std::string::npos)
			while (!new_subkeyword)
			{
				index1 = (size_t)rfd_file->tellg();
				line_str1 = readNonBlankLineFromInputStream(*rfd_file);
		...
		// subkeyword found
		if (line_string.find("$ISOTOPE_FRACTIONATION") != std::string::npos)
		{
		...
		//....................................................................
		// subkeyword found
		if (line_string.find("$EXCHANGE_PARAMETERS") != std::string::npos)
		{
			in.str(readNonBlankLineFromInputStream(*rfd_file));
			in >> dval;   // velocity
...
int CKinReact::CheckReactionDataConsistency(std::vector<CKinBlob*> & KinBlob_vector)
{
	...
	if ((fabs(rateconstant) < MKleinsteZahl) && (type.compare("monod") == 0))
	{
		...
		ok = 0;
		std::cout << " Rateconstant is zero" << std::endl;
	}
	...
	{
		ok = 0;
		std::cout << " Not enough reaction partners" << std::endl;
		for (i = 0; i < length; i++)
			std::cout << reactionpartner[i] << "  ";
		std::cout << std::endl;
	}
	...
	{
		ok = 0;
		std::cout << " Not enough stochieometric coefficients for reaction partners"
		     << std::endl;
		for (i = 0; i < length; i++)
			std::cout << stochmet[i] << "  ";
		std::cout << std::endl;
...
void CKinReact::ReadReactionEquation(string line_string_all)
{
	...
	if (p != number_reactionpartner)
		...
		<< "The equation making trouble is: \n" 
		<< line_string 
		<< endl;
}
...
bool CKinBlob::Read(ifstream* rfd_file)
{
	...
	{
		index = (size_t)rfd_file->tellg();
		if(!rfd_file->getline(line,MAX_ZEILE)) 
			break;
		// if (!GetLineFromFile(line, rfd_file))
		//  	break;
		line_string = line;
...
//					ReactionDeactivation(nnodes);
//			// Check if nodes should be deactivated or activated for this time step
//
//			unsigned count = 0;
